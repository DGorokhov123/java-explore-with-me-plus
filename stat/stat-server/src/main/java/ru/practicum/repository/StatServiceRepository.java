...
public interface StatServiceRepository extends JpaRepository<Stat, Long> {
    ...
    @Query("select new ru.practicum.dto.EventStatsResponseDto(stat.ip, stat.uri, count(distinct stat.ip)) " +
            "from Stat as stat " +
    ...
    @Query("select new ru.practicum.dto.EventStatsResponseDto(stat.ip, stat.uri, count(stat.ip)) " +
            "from Stat as stat " +
            "where stat.timestamp between ?1 and ?2 " +
            "group by stat.ip, stat.uri " +
